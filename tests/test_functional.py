...
from src.mfc_cli import main as cli_main
from src.daemon_main import main as daemon_main

# Mark all tests in this file as requiring root privileges
pytestmark = pytest.mark.skipif(os.geteuid() != 0, reason="Functional tests require root privileges")
...
def netns_env(tmp_path):
    ...
    try:
        ...
        daemon_cmd = [
            ...
            sys.executable, "-m", "src.daemon_main", # Run as a module
        ...
        if os.path.exists(f"/var/run/netns/{ns_name}"):
            subprocess.run(["ip", "netns", "del", ns_name], check=False, capture_output=True)
